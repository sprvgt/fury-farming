...
/*
        ...
        for (unordered_set<string>::iterator it = wordDict.begin();
            it != wordDict.end(); ++ it)
        {
            if (oneDiff(beginWord, *it))
            {
                if (edges.count(beginWord))
                    edges[beginWord].push_back(*it);
                else
                {
                    vector<string> tmp;
                    tmp.push_back(*it);
            ...
            if (reach.count(*it) && *it != beginWord)
                for (unordered_set<string>::iterator jt = wordDict.begin();
                    jt != wordDict.end(); ++ jt)
                {
                    if (oneDiff(*it, *jt))
                    {
                        if (edges.count(*it))
                            edges[*it].push_back(*jt);
                        else
                        {
                            vector<string> tmp;
                            tmp.push_back(*jt);
        ...
        bool flag = false;
        for (unordered_set<string>::iterator it = reach.begin();
            it != reach.end(); ++ it)
            if (oneDiff(*it, endWord))
            {
                flag = true;
                edges[*it].push_back(endWord);
            }
        if (!flag)  return 0;
*/            
        ...
        while(!que.empty())
        {
            ...
            if (edges.count(str))
            {
                ...
                for (int i = 0; i < l; i ++)
                    ...
                    {
                        if (edges[str][i] == endWord)
                            return dep + 1;
                        que.push(make_pair(edges[str][i], dep + 1));
                        visited.insert(edges[str][i]);
                    }
            }
            que.pop();
        }
...
cout << Solution().ladderLength("sand", "acne", input) << endl;
return 0;
}
